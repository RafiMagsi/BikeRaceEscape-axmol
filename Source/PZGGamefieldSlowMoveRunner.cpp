//
//  PZGGamefieldSlowMoveRunner.cpp
//  ProjectMayhem
//
...
Scene* PZGGamefieldSlowMoveRunner::scene(){
    
	Scene *scene = Scene::create();
	PZGGamefieldSlowMoveRunner *layer = PZGGamefieldSlowMoveRunner::create();
	scene->addChild(layer);
	return scene;
}

void PZGGamefieldSlowMoveRunner::reset(){
    ...
    gf_speed_base = gameInfoObject->gameSpeedMin;
    ...
    platform->platfromsBetweenDistance = 0;
    platform->isSlowMoveMode = true;
    
    
    platform->speed = gf_speed;
    platform->update( 1/30.0 );
    platform->update( 1/30.0 );
    platform->update( 1/30.0 );    
}


bool PZGGamefieldSlowMoveRunner::init()
{
	if ( !Layer::init() )	{
		return false;
	}
...
void PZGGamefieldSlowMoveRunner::update(float dt){
    if (tip_counter <= 3 ){
        tip_counter += dt;
...
void PZGGamefieldSlowMoveRunner::ccTouchesBegan(__Set *pTouch, Event *pEvent){
    isMoved = true;
    gf_speed = gf_speed_base;
}

void PZGGamefieldSlowMoveRunner::ccTouchesEnded(__Set *pTouch, Event *pEvent){
    isMoved = false;
    gf_speed = 0;
}